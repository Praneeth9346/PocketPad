...
(() => {
  ...
  function connectWebSocket() {
    ...
    socket.onopen = () => {
      ...
      isConnected = true;
      const isUsb = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
    ...
    socket.onmessage = (event) => {
      if (event.data instanceof ArrayBuffer) {
        ...
        // Handle json messages here if needed in the future
      }
    };
  ...
  function handleDirectDeviceMotion(e) {
    ...
    const visualSteerDeg = smoothedAngle * steeringSensitivity;
  ...
  function updateSteeringSensitivity(sensMultiplier) {
    ...
    sensPills.forEach(pill => {
      ...
      if (Math.abs(pSens - steeringSensitivity) < 0.001) {
        pill.classList.add('active');
      } else {
        pill.classList.remove('active');
      }
  ...
  function setupPedals() {
    ...
    setupPedalZone(throttleZone, (val) => {
      if (val === 1.0 && lastThrottle < 1.0) triggerHaptic(30);
      lastThrottle = val;
      currentThrottle = val;
    ...
    setupPedalZone(brakeZone, (val) => {
      if (val === 1.0 && lastBrake < 1.0) triggerHaptic(30);
      lastBrake = val;
      currentBrake = val;
      // GPU Composited scaleY Transform (Zero Layout Reflows)
  ...
  function setupPedalZone(zoneElem, onValueChange) {
    ...
    const onRelease = (e) => {
      ...
      e.preventDefault();
      pointerId = null;
      zoneElem.classList.remove('active');
